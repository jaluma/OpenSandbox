...
| server.image | object | `{"repository":"sandbox-registry.cn-zhangjiakou.cr.aliyuncs.com/opensandbox/server","tag":"v0.2.3"}` | Server image configuration |
...
| server.image.tag | string | `"v0.2.3"` | Server image tag. Defaults to the chart appVersion when empty. |
